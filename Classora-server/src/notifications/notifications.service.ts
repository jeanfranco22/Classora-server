...
export class NotificationsService {
  async sendWelcomeEmail(name: string, email: string) {
    await transporter.sendMail({
      from: '"Classora" <gympower.ok@gmail.com>',
      to: email,
      subject: '¡Bienvenido a Classora! 💪',
      html: `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h1 style="color: #ff6600; font-size: 28px;">¡Bienvenido a Classora, ${name}!</h1>
            ...
            <p style="font-size: 14px; color: #888;">El equipo de Classora</p>
            </div>`,
    });
  }
  ...
  async inactiveUserEmail(name: string, email: string) {
    await transporter.sendMail({
      ...
      html: `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p style="font-size: 16px; line-height: 1.6;">Hola <strong>${name}</strong>, tu cuenta en Classora ha sido desactivada.</p>
  ...
  async promoteTeacherEmail(name: string, email: string) {
    await transporter.sendMail({
      from: '"Classora" <gympower.ok@gmail.com>',
      to: email,
      subject: 'Bienvenido al equipo docente de Classora',
      html: `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h1 style="color: #2f6fed; font-size: 26px;">Felicitaciones, ${name}</h1>
            <p style="font-size: 16px; line-height: 1.6;">Te damos la bienvenida como profesor oficial de Classora.</p>
            <p style="font-size: 16px; line-height: 1.6;">A partir de ahora tendrás acceso a las herramientas para gestionar clases y acompañar estudiantes.</p>
            <p style="font-size: 14px; color: #888;">El equipo de Classora</p>
          </div>`,
    });
  }
  ...
  async sendExpiredMembershipEmail(name: string, email: string) {
    await transporter.sendMail({
      from: '"Classora" <gympower.ok@gmail.com>',
      to: email,
      subject: 'Tu membresía en Classora ha vencido',
      html: `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #cc0000;">Tu membresía ha vencido</h2>
      <p style="font-size: 16px; line-height: 1.6;">Hola <strong>${name}</strong>, te informamos que tu membresía en Classora ha vencido.</p>
  ...
  async sendMembershipExpiringSoonEmail(name: string, email: string) {
    await transporter.sendMail({
      from: '"Classora" <gympower.ok@gmail.com>',
      to: email,
      subject: '⚠️ Tu membresía en Classora está por vencer',
      html: `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #ff6600;">Tu membresía vence en 3 días</h2>
      <p style="font-size: 16px; line-height: 1.6;">Hola <strong>${name}</strong>, te recordamos que tu membresía en Classora vence en <strong>3 días</strong>.</p>
  ...
  async sendActiveUsersReport(users: Omit<User, 'password'>[], email: string) {
    await transporter.sendMail({
      from: '"Classora" <gympower.ok@gmail.com>',
      to: email,
      subject: '📊 Reporte de usuarios activos — Classora',
